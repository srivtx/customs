...
/**
 * why.tsx — the "why it exists" page: the problem in plain words, the
 * principle (machines get mandates, not cards), the architecture on one
 * diagram, and the honest scope ledger — what shipped, what is simulated,
 * what is not built yet. Written to be read, not scanned.
 */
import { GhostButton, InkButton, Reveal, SectionLabel, Stamp, TierChip } from "./bits";
...
export function WhyPage({ onEnter }: { onEnter: (v: View) => void }) {
  return (
    <div className="space-y-14">
      {/* ------------------------------ the claim ------------------------------ */}
      <section aria-label="why customs exists">
        <div className="max-w-3xl">
          <div className="label-caps">why customs exists</div>
          <h1 className="mt-4 font-display text-[clamp(34px,5vw,54px)] font-medium leading-[1.05] tracking-tight text-ink">
            Every checkout on the internet assumes a human is paying.
          </h1>
          <p className="mt-5 text-[15.5px] leading-relaxed text-inksoft">
            Cards have CVV prompts and OTP screens. Wallets want a face or a fingerprint.
            UPI wants a PIN pushed to a phone a person is holding. Every one of these
            friction points exists to answer one question — <em className="text-ink">are you
            really the person authorized to spend this?</em> — and every one of them
            answers it by assuming the payer has hands, eyes, and a phone.
          </p>
          <p className="mt-4 text-[15.5px] leading-relaxed text-inksoft">
            AI agents break that assumption. An agent shopping for its principal is
            fast, tireless, and literate — and cannot type a PIN, press a thumb, or
            vouch for itself. So today the agent does the easy part (find the product,
            compare, decide) and stops dead at the money. A human finishes the
            transaction. The checkout is the wall.
          </p>
          <p className="mt-4 text-[15.5px] leading-relaxed text-inksoft">
            The industry answer on the horizon — agent payment protocols, NPUs opening
            rails to non-human payers — moves the wall, it does not remove it. If an
            agent can pay, the question stops being <em className="text-ink">is the payer
            human?</em> and becomes <em className="text-ink">is this payment authorized,
            bounded, and provable?</em> That question has an old, solved shape. It is
            the question a customs house answers a thousand times a day.
          </p>
        </div>
      </section>
      ...
      {/* ------------------------------ the principle ------------------------------ */}
      <Reveal>
        <section className="doc border-line2 px-5 py-5" aria-label="the principle">
          <SectionLabel>the principle — machines get mandates, not cards</SectionLabel>
          <p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-inksoft">
            A customs house never asks the cargo whether it is honest. It asks for the
            manifest, checks it against the rules, stamps what passes, and refuses what
            does not — every decision written down. Customs applies exactly that
            protocol to machine spending: the agent does not get a payment credential.
            It gets a <span className="font-semibold text-ink">mandate</span> — a signed
            envelope that says how much, for which items, until when, at which trust
            level — and every rupee the agent moves is checked against that envelope
            in plain, auditable code.
          </p>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={p.t} className="rounded-sm border border-line bg-paper2/40 px-4 py-3">
                <div className="label-caps">{p.t}</div>
                <p className="mt-1.5 text-[13px] leading-relaxed text-inksoft">{p.d}</p>
              </div>
            ))}
          </div>
          <div className="mt-5 flex flex-wrap items-center gap-2">
            <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-inksoft">the trust ladder:</span>
      ...
      <Reveal>
        <section aria-label="architecture">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <SectionLabel>the architecture — one gate, two counters</SectionLabel>
            <span className="font-mono text-[10px] text-inksoft">
              the full decision table lives in ARCHITECTURE.md · every box below is code you can open
            </span>
          </div>
              ...
              {[
                ...
                <div key={n} className="bg-card px-4 py-3.5">
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="font-display text-[15px] font-medium text-ink">{t}</span>
                  </div>
      ...
      <Reveal>
        <section aria-label="what exists and what does not">
          <SectionLabel>the scope ledger — shipped, simulated, not yet</SectionLabel>
          <p className="mt-4 max-w-3xl text-[14px] leading-relaxed text-inksoft">
            We hold ourselves to the brief&apos;s own bar — every money action
            explainable, bounded and gated — and to a rule set in AGENTS.md: nothing
            ships unlabeled. If a number is in this product, it regenerates from a
            command. If a path is simulated, it says so on the screen. Here is the
            whole truth of what exists.
          </p>
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            <div className="doc px-4 py-4">
              <Stamp kind="cleared" animate={false}>SHIPPED · TESTABLE</Stamp>
              <ul className="mt-3 space-y-2 text-[12.5px] leading-relaxed text-inksoft">
                {[
                  ...
                  <li key={s} className="flex gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rotate-45 bg-cleared" aria-hidden />
                    <span>{s}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="doc px-4 py-4">
              ...
              <ul className="mt-3 space-y-2 text-[12.5px] leading-relaxed text-inksoft">
                {[
                  ...
                  <li key={s} className="flex gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rotate-45 bg-held" aria-hidden />
                    <span>{s}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="doc px-4 py-4">
              ...
              <ul className="mt-3 space-y-2 text-[12.5px] leading-relaxed text-inksoft">
                {[
                  ...
                  <li key={s} className="flex gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rotate-45 bg-refused" aria-hidden />
                    <span>{s}</span>
                  </li>
                ))}
              </ul>
      ...
      {/* ------------------------------ where to go ------------------------------ */}
      <Reveal>
        <section className="flex flex-wrap items-center gap-3 border-t border-line pt-6" aria-label="continue">
...
/* ------------------------------------------------------------------ */
/* the architecture diagram — drawn, not screenshotted                 */
/* ------------------------------------------------------------------ */
...
function ArchDiagram() {
  ...
  const boxStyle = { fill: "#fdfbf6", stroke: "#c2b9a1", strokeWidth: 1.5 };
  const gateStyle = { fill: "#fdfbf6", stroke: "#1b180f", strokeWidth: 2 };
  const caps = (x: number, y: number, s: string, size = 10, fill = soft, ls = 1.6) => (
  ...
  return (
    ...
    >
      ...
      {/* buyer box */}
      <rect x={30} y={44} width={240} height={252} rx={4} style={{ fill: "none", stroke: line, strokeDasharray: "3 4", strokeWidth: 1.5 }} />
      ...
      <path d="M150 120 L150 140" stroke={line} strokeWidth={1.5} markerEnd="url(#arrow)" />
      <path d="M150 194 L150 214" stroke={line} strokeWidth={1.5} markerEnd="url(#arrow)" />

      {/* mandate approval loop (human in the loop) */}
      <path d="M246 93 C 300 93 310 120 356 120" fill="none" stroke="#8f5e0a" strokeWidth={1.5} markerEnd="url(#arrow)" strokeDasharray="5 3" />
      {caps(300, 63, "mandate approval", 9, "#8f5e0a", 1.2)}
      ...
      {[
        ...
        <g key={s}>
          <rect x={386} y={96 + i * 30} width={288} height={24} rx={2} fill="#efeae0" />
          <circle cx={400} cy={108 + i * 30} r={3.5} fill="#175e43" />
          <text x={414} y={112 + i * 30} fontSize={10.5} fill={ink} style={monoF} letterSpacing={0.4}>
            {s}
          </text>
        </g>
      ))}
      <path d="M246 241 C 300 241 310 190 360 186" fill="none" stroke={ink} strokeWidth={2} markerEnd="url(#arrowInk)" />
      ...
      <path d="M700 172 L742 172" stroke={ink} strokeWidth={2} markerEnd="url(#arrowInk)" />

      {/* merchant box */}
      <rect x={890} y={44} width={140} height={252} rx={4} style={{ fill: "none", stroke: line, strokeDasharray: "3 4", strokeWidth: 1.5 }} />
      ...
      <path d="M858 172 L890 172" stroke={ink} strokeWidth={2} markerEnd="url(#arrowInk)" />

      {/* ledger bar — spans everything */}
      <rect x={30} y={368} width={1000} height={76} rx={4} fill="#efeae0" stroke={line} strokeWidth={1.5} />
      ...
      {Array.from({ length: 12 }, (_, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={414} width={26} height={22} rx={2} fill="#fdfbf6" stroke={soft} strokeWidth={1.2} />
            <text x={x + 13} y={429} fontSize={8.5} fill={soft} style={monoF} textAnchor="middle">
              {`#${i + 1}`}
            </text>
            {i < 11 && <path d={`M${x + 26} 425 L${x + 76} 425`} stroke={line} strokeWidth={1.2} />}
          </g>
        );
      })}
      {/* arrows into the ledger */}
      <path d="M150 296 L150 368" stroke={line} strokeWidth={1.5} markerEnd="url(#arrow)" />
      <path d="M530 330 L530 368" stroke={ink} strokeWidth={2} markerEnd="url(#arrowInk)" />
      <path d="M960 296 L960 368" stroke={line} strokeWidth={1.5} markerEnd="url(#arrow)" />
